...
**OmniRoute** — unified AI proxy/router. One endpoint, 297 LLM providers, auto-fallback.
...
This project is indexed by GitNexus as **OmniRoute** (124458 symbols, 245936 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **MUST run `detect_changes()` before committing** to verify your changes only affect expected symbols and execution flows. For regression review, compare against the default branch: `detect_changes({scope: "compare", base_ref: "less"})`.
