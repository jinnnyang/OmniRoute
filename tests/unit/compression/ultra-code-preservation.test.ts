 ...
 * `add(a, b) { return a + b; }` into `add(a, return`. caveman extracts/restores
 * preserved blocks first; ultra must too.
 */
import { test } from "node:test";
import assert from "node:assert/strict";
import { ultraCompressHeuristic } from "@omniroute/open-sse/services/compression/ultra.ts";

test("ultraCompressHeuristic preserves fenced code, inline code, and URLs byte-identical", () => {
  ...
  const { messages } = ultraCompressHeuristic([{ role: "user", content: text }], {
